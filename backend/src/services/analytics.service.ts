...
import { convertCentsToKes } from "../utils/format-currency";

//convertToDollarUnit was replaced with convertCentsToKes in the format-currency util, but I kept the name here for clarity in the service layer. We can consider renaming it to convertCentsToDollarUnit for consistency across the codebase in the future.
...
export const summaryAnalyticsService = async (
  ...
  return {
    availableBalance: convertCentsToKes(availableBalance),
    totalIncome: convertCentsToKes(totalIncome),
    totalExpenses: convertCentsToKes(totalExpenses),
    savingRate: {
    ...
    percentageChange: {
      ...percentageChange,
      previousValues: {
        incomeAmount: convertCentsToKes(
          percentageChange.previousValues.incomeAmount,
        ),
        expenseAmount: convertCentsToKes(
          percentageChange.previousValues.expenseAmount,
        ),
        balanceAmount: convertCentsToKes(
...
export const chartAnalyticsService = async (
  ...
  const transaformedData = (resultData?.chartData || []).map((item: any) => ({
    date: item.date,
    income: convertCentsToKes(item.income),
    expenses: convertCentsToKes(item.expenses),
  }));
...
export const expensePieChartBreakdownService = async (
  ...
  const transformedData = {
    totalSpent: convertCentsToKes(data.totalSpent),
    breakdown: data.breakdown.map((item: any) => ({
      ...item,
      value: convertCentsToKes(item.value),
    })),
  };
